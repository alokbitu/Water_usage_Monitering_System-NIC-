...
# Filter zip files starts with site_id, station_name in SUNJRAY
def filter_zipfiles_with_site_details():
    zipfiles_in_sunjray = []
    for file in os.listdir(sunjraydir):
        if file.startswith(fsw) & file.endswith(".zip"):
            zipfiles_in_sunjray.append(file)
    return zipfiles_in_sunjray


# Delete real time timeout zip files in SUNJRAY Realtime directory after 10 minutes.
# List all files in SUNJRAY Realtime directory(sunjray_realtime_path).
# Filter zip files with site_id and station_name in SUNJRAY Realtime directory.
...
def del_realtime_timeout_zipfiles():
    list_of_zipfiles = []
    allfiles = os.listdir(sunjray_realtime_path)
    ...
    for i in range(0, len(list_of_zipfiles)):
        ...
        if minutes[0] > 2:
            try:
                zipfile_path = os.path.join(sunjray_realtime_path, name_of_zipfile)
                print(zipfile_path)
                os.unlink(zipfile_path)
                print("File deleted in " + sunjraydir + "/Realtime- ", name_of_zipfile)
            except:
                print("Error while deleting file in " + sunjraydir + "/Realtime- ", name_of_zipfile)


# copies the files from SUNJRAY to Realtime
def move_zipfiles_to_realtime():
    zipfif = filter_zipfiles_with_site_details()
    for i in range(0, len(zipfif)):
        # Extracting zip file time
        zfln = zipfif[i]
        filepath = os.path.join(sunjray_path, zipfif[i])
        fmin = int(zfln[-8:-6])
        fhrs = int(zfln[-10:-8])
        fday = int(zfln[-12:-10])
        fmon = int(zfln[-14:-12])
        ...
        if 0 <= minutes[0] <= 2:
            cpth = os.path.join(sunjray_realtime_path, zfln)
            if os.path.isfile(cpth):
                # print("File exists")
                pass
            else:
                newPath = shutil.copy(filepath, sunjray_realtime_path)
...
def zipfile_for_realtime_upload():
    try:
        ...
        for x in os.listdir(sunjray_realtime_path):
            zipfiles.append(x)
        ...
        # print("No files in "+sunjraydir+"/"+rtdir+ " for realtime upload")
        return -2


# Read upload zipfile
def read_zipfile(zfln):
    filepath = os.path.join(sunjray_realtime_path, zfln)
    f = open(filepath, "rb")
    while True:
        data = f.read()
        if not data:
...
def add_boundaries_to_zipfile_data(upload_zipfile_name):
    try:
        os.unlink("sunjrayruf.txt")
        # print("File Deleted: sunjrayruf.txt")
    except:
        pass
        # print("Error while deleting file: sunjrayruf.txt")
...
Content-Type:application/zip\r\n\r\n'.format(upload_zipfile_name)
    ...
    file1 = open("sunjrayruf.txt", "wb")  # append mode
...
# Reads data from sunjrayruf.txt
def read_data_to_upload():
    fileName = "sunjrayruf.txt"
    f = open(fileName, "rb")
    while True:
        data = f.read()
        if not data:
...
# Deletes the zip file from SUNJRAY/Realtime folder after getting Success message from the server.
def sunjray_realtime_upload(upload_zipfile_name):
    auth, sign, ts = sunjray_authorization()
    header = {
    ...
    # Upload data
    f = open('sunjrayruf.txt', 'rb')
    mydata = f.read()
    f.close()

    try:
        print("\n**************SUNJRAY Realtime upload****************")
        ...
        if servermsg["status"] == "success":
            print("\n*********SUNJRAY Realtime Upload Response**********")
            print("Successfully uploaded Realtime file Name: ", upload_zipfile_name)
            print(resp.text)
            #data = json.loads(resp.text)
            #print(data)
            try:
                rmzfit = os.path.join(sunjraydir, upload_zipfile_name)
                os.unlink(rmzfit)
                print("Deleted Real time uploaded file in " + sunjraydir + ": ", upload_zipfile_name)
            except FileNotFoundError:
                print("File already deleted")
            try:
                rmzfit = os.path.join(sunjray_realtime_path, upload_zipfile_name)
                os.unlink(rmzfit)
                print("Deleted Real time uploaded file in " + sunjraydir + "/Realtime: ", upload_zipfile_name)
            except:
                print("Error while deleting Real time uploaded file in " + sunjraydir + "/Realtime: ", upload_zipfile_name)
        else:
            #print(servermsg)
            time.sleep(10)
...
def sunjrayrumain():
    ...
    if upload_zipfile_name != -2:
        ...
        sunjray_realtime_upload(upload_zipfile_name)
        return 1
    else:
        return 0
